...
class ImageProcessor(threading.Thread):
    ...
    def run(self):
        while True:
            try:
                ...
                result = self.perform_operation(image, operation)
    ...
    def perform_operation(self, image, operation):
        try:
            if operation == 1:
                return self.detect_edges(image)
            elif operation == 2:
                return self.blur_image(image)
            elif operation == 3:
                return self.convert_to_grayscale(image)
            elif operation == 4:
                return self.invert_colors(image)
            else:
    ...
    def detect_edges(self, img, threshold1=90, threshold2=180):
        try:
    ...
    def blur_image(self, img, ksize=(5, 5)):
        try:
...
def server_main(comm):
    ...
    while True:
        try:
            ...
            healthy_workers = [i for i in range(1, num_workers) if is_instance_healthy(i)]
            ...
            send_processed_images(client_socket, processed_images)
        except Exception as e:
...
def worker_main(comm):
    try:
        while True:
            ...
            if task is None:
                break
            processor = ImageProcessor(comm, task)
            processor.start()
            processor.join()
    except Exception as e:
        print(f"Worker error: {e}")

def send_processed_images(client_socket, images):
    try:
